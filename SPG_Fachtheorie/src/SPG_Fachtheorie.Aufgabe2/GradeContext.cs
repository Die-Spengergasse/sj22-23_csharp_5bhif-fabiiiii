...
namespace SPG_Fachtheorie.Aufgabe2
{
    public class GradeContext : DbContext
    {
        ...
        public void Seed()
        {
            ...
            {
                var lastname = f.Name.LastName();
                List<bool?> a = new List<bool?>() { true, false, null };
                return new Student
                {
                    ...
                    ConferenceDecision = f.Random.ListItem(a)
                };
            })
            .Generate(120)
            .GroupBy(s => s.Email)
            ...
            {
                if (faker.Random.Bool()) { continue; }
                exams.Add(new Exam
                {
                    Date = faker.Date.Between(faker.Date.Recent(), faker.Date.Soon()),
                    Grade = g,
                });
            }
